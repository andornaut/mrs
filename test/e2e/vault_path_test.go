...
func TestSearchAndExportReadAVaultNamedByPath(t *testing.T) {
	...
	l.Run("search", "--file", p, "-p", pwFile, "github").
		AssertOK().
	...
	l.Run("export", "--file", p, "-p", pwFile).AssertOK().AssertStdout("token: abc123")
}
...
func TestEditWritesAVaultNamedByPathWhereItIs(t *testing.T) {
	...
	l.Run("edit", "--file", p, "-p", pwFile).AssertOK().AssertStderr("Saved changes to vault " + p)

	l.Run("export", "--file", p, "-p", pwFile).AssertOK().AssertStdout("key: xyz")
	// The lock is the vault's sibling, so it is written in the directory the
	// vault is in rather than in the vault directory.
...
func TestAVaultNamedByPathIsNotLookedForInTheVaultDirectory(t *testing.T) {
	...
	l.Run("export", "--file", p, "-p", pwFile).AssertOK().AssertStdoutExactly("elsewhere\nvalue: outside\n")
...
func TestNamingAVaultByNameAndByPathIsAWrongInvocation(t *testing.T) {
	...
	for _, args := range [][]string{
		{"add", "--file", p, "-v", "work"},
		{"edit", "--file", p, "-v", "work"},
		{"export", "--file", p, "-v", "work"},
		{"search", "--file", p, "-v", "work", "github"},
	} {
		l.Run(append(args, "-p", pwFile)...).
			AssertUsageError().
			AssertStderr("--vault and --file both name a vault; use one").
...
func TestAPathThatDoesNotNameAVaultFileIsRefused(t *testing.T) {
	...
	for _, path := range []string{plain, filepath.Join(filepath.Dir(p), "work")} {
		l.Run("export", "--file", path, "-p", pwFile).
			AssertFailed().
			AssertStderr("does not name a vault").
			AssertStderr("<name>.<salt>")
	}
	l.Run("export", "--file", filepath.Join(filepath.Dir(p), "gone."+filepath.Ext(p)[1:]), "-p", pwFile).
		AssertFailed().
