...
- Vaults written with the earlier 4,096 iterations are not read. `mrs` says so
  when it meets one, rather than reporting the wrong password it cannot tell it
  from. Every release up to v0.1.7 reads one and re-encrypts it at 600,000
  iterations on the next save, so open and save such a vault with one of those.
